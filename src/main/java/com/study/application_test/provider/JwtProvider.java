package com.study.application_test.provider;
...
/*
=== JwtProvider 클래스 ===
: JWT(JSON Web Token) 토큰을 생성하고 검증하고 역할

cf) JWT
    : 사용자 정보를 암호화된 토큰으로 저장, 서버에 요청할 떄마다 전달 가능 (사용자 정보 확인용)
    : 주로 로그인 인증에 사용
...
@Bean vs @Component
    ...
    @Component: 클래스 레벨에서 선언, 스프링 컨타임 시 컴포넌트 스캔을 통해 자동으로 빈을 찾고 등록 (의존성 주입)
 */
...
public class JwtProvider {
    ...
    public String generateJwtToken(String username, Set<String> roles) {
        return Jwts.builder()
                ...
                .claim("roles", roles)
                .setIssuedAt(new Date()) // 현재 시간을 기준으로 토큰 발생 시간 설정
    ...
    public String removeBearer(String bearerToken) {
        ...
        // substring(A, B): A 이상 B 미만까지의 문자열을 리턴
    ...
    public Set<String> getRolesFromJwt(String token) {
        Claims claims = getClaims(token);
        return new HashSet<>((List<String>) claims.get("roles")); // List 형태로 가져와서 Set 으로 반환
    }
}
